...
%% Parse inputs
    % Create figure
    fig_now = figure('Units','pixels','Position',[50 50 210.*3 297.*3],'visible','off');
...
%% >>>>>>>>>> Example cells
    ...
    for ii = 1:numel(ucis)
        ...
        ax1 = axes('Units','pixels','Position',[x(jj) y(jj) xsiz(1) ysiz(1)]);
            ...
            text(1,0,sprintf('%d',ii),'Units','normalized','HorizontalAlignment','right','FontSize',7,'Color','k','VerticalAlignment','top')
        ...
        count = count+1;
        if count==24 % max we can fit on a page
            disp(sprintf('...saving'))    
            if 1
                [~,~,~] = mkdir([config.media_out_dir '\all_place_cells\']);
                fname = [config.media_out_dir '\all_place_cells\fig_' num2str(fig_count) '.png']; 
