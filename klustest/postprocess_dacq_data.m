...
for ll = 1:3
    % get data
    pox = double(mtint.pos.led_pos(:,1,ll));
    poy = double(mtint.pos.led_pos(:,2,ll));
    pot = double(mtint.pos.ts(:));
    ...
    mtint.pos.led_pos(:,1,ll) = single(pox);
    mtint.pos.led_pos(:,2,ll) = single(poy);  
end
...
for ll = 1:3
    ...
    [datout,~,~] = smoothn({mtint.pos.led_pos(:,1,ll),mtint.pos.led_pos(:,2,ll)},100,'robust');
    mtint.pos.led_pos(:,1,ll) = datout{1};
    mtint.pos.led_pos(:,2,ll) = datout{2};       
end
...
% pox = mtint.pos.led_pos(:,1);
% poy = mtint.pos.led_pos(:,2);
% 
...
%% Calculate instantaneous speed using smoothed data, this should contain the correct number of zero values and no values exceeding maxspeed
    rpox = mtint.pos.led_pos(:,1,1);
    rpoy = mtint.pos.led_pos(:,2,1);
    gpox = mtint.pos.led_pos(:,1,2);
    gpoy = mtint.pos.led_pos(:,2,2);
    bpox = mtint.pos.led_pos(:,1,3);
    bpoy = mtint.pos.led_pos(:,2,3);

    pox = mean([rpox gpox bpox],2); % rat X center of mass
    poy = mean([rpoy gpoy bpoy],2); % rat Y center of mass
