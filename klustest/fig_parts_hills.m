function fig_parts_hills(mtint,pdata,sdata)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% FUNCTION BODY
    fig_clust = figure('visible',fig_vis,'Units','pixels','Position',[50, 50, 1850, 920]); % open/create figure
    ...
    ann_str = sprintf('Cell: %s, Rat: %s, Date: %d, Tetrode: %d, Cluster: %d, Part: %s, Analysed: %s',sdata.uci{1},sdata.rat{1},sdata.date,sdata.tet,sdata.clu,part_now,datestr(now,'yyyy-mm-dd-HH-MM-SS'));
...
%% ################################################################# %% Position and spike plot (black lines & red dots)
    ax_ps = axes('Units','pixels','Position',[30,560,350,200]);
        ...
        pspx = ppox(sdata.spike_index{1});
        pspy = ppoy(sdata.spike_index{1});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ################################################################# %% 16 spike plots showing consistency over time
    if config.temp_asis
        % work out how much time/samples must be shown per plot
        total_samples = numel(ppot);
        samples_per_plot = ceil(total_samples / 16); % a minimum of 60s per plot
...
%% ################################################################# %% Spike rasterplot running along bottom of figure
    if config.temp_asis
        axsr = axes('Units','pixels','Position',[30,35,1750,45]);
...
%% ################################################################# %% Firing rate map
    if config.fild_asis
        axrt = axes('Units','pixels','Position',[480,560,350,200]);
            ratemap = sdata.ratemap_planar{1};
            spati = sdata.spatial_info_bsec(:,1);
            spars = sdata.sparsity(:,1);
            coher = sdata.spatial_coherence(:,1);
...
%% ################################################################# %% Dwell time map
    if config.fild_asis
        axdt = axes('Units','pixels','Position',[1400,560,350,200]);
            dwellmap = pdata.(part_now).dwellmap_planar;

            im = imagesc(dwellmap);
            colormap(axdt,hot(256))    
            set(im,'alphadata',dwellmap>0);
            daspect([1 1 1])
            ...
            tt = text(-0.05,0.5,sprintf('Max: %.2f, Mean: %.2f',nanmax(dwellmap(:)),nanmean(dwellmap(:))),'FontSize',fsiz,'Units','normalized','HorizontalAlignment','center');
            set(tt,'rotation',90);  
            ...
            set(gca,'Position',axp);
            set(cc,'Position',get(cc,'Position')+[-0.004 0 -0.004 0])
            title(cc,'Time (s)','FontSize',fsiz)            
    end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ################################################################# %% Grid autocorrelation
    if config.grid_asis
        axgs = axes('Units','pixels','Position',[930,560,350,200]);
            amap = sdata.amap_planar{1};    
            msk = sdata.amap_planar_mask{1};
            if isnan(msk)
                msk = zeros(size(amap));
            end
            msk = ones(size(amap));
...
%% ################################################################# %% Waveform plots      
    if config.wave_asis
        axes('Units','pixels','Position',[50,350,200,170])
            %wavtime = -200:20:780;
            ...
            hbuff = 205;
            wpos = [ax.Position(1)+hbuff,ax.Position(2)+120,50,50; ax.Position(1)+hbuff,ax.Position(2)+70,50,50; ax.Position(1)+hbuff,ax.Position(2)+20,50,50; ax.Position(1)+hbuff,ax.Position(2)-30,50,50];
            for ww = 1:4
...
%% ################################################################# %% Interspike interval  
    if config.sisi_asis
        axes('Units','pixels','Position',[50,140,250,120])
            xi = 0:0.5:50;
            ...
            text(.5,1.07,sprintf('fwhmx: %.1fms, 10ms proportion: %.2f',sdata.isi_fwhmx,nansum(idist(xi<=10))./numel(pspt)),'Units','normalized','FontSize',fsiz,'HorizontalAlignment','center')
            grid on;
    end    
...
%% ################################################################# %% Spike autocorrelation 1 - 500ms theta
        ax1 = axes('Units','pixels','Position',[370,370,250,150]);
...
%% ################################################################# %% Spike autocorrelation 2 - 25ms refractory period   
    ax2 = axes('Units','pixels','Position',[ax1.Position(1),ax1.Position(2)-230,ax1.Position(3),150]);
            autoc = sdata.t25_spike_autocorr{1};
            tlag = -24.75:0.5:24.75;

            bar(tlag,autoc,1,'k'); hold on;
            tau_r = 2; % length of refractory period in ms
            plot([-tau_r; -tau_r],ax2.YLim,'r','LineWidth',1)
            plot([tau_r; tau_r],ax2.YLim,'r','LineWidth',1) 
            plot([-6; -6],ax2.YLim,'r:','LineWidth',0.5)
            plot([6; 6],ax2.YLim,'r:','LineWidth',0.5) 
            axis manual
            ...
            set(ax2,'yticklabel',num2str(get(gca,'ytick')','%.2f'))
    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ################################################################# %% Cluster quality
    ax1 = axes('Units','pixels','Position',[690,400,230,120]);
        ...
        if ~isempty(cdat) && ~isempty(ndat)
            ...
            text(.5,1.08,sprintf('Iso-D: %.2f, L-ratio: %.2f',isod,lrat),'Units','normalized','FontSize',fsiz,'HorizontalAlignment','center')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ################################################################# %% Cluster space
    ax2 = axes('Units','pixels','Position',[ax1.Position(1),ax1.Position(2)-260,ax1.Position(3),170]);
        ...
        text(.5,1.09,sprintf('Ch%d vs Ch%d: %d clusters\n%d spikes (%.1f%% of total %d)',mch1,mch2,clus_count,numel(pspx),numel(pspx)/tspikes*100,tspikes),'Units','normalized','FontSize',fsiz,'HorizontalAlignment','center')
...
%% ################################################################# %% Head direction (linear)
    ax1 = axes('Units','pixels','Position',[980,400,230,120]);   
        ...
        ai = [ai(:)-360;ai(:);ai(:)+360];
        hd_ratemap = repmat(hd_ratemap(:),3,1);
        hd_dwellmap = repmat(hd_dwellmap(:),3,1);
        ...
        ax1.YAxis(1).Color = 'k';
        ax1.YAxis(2).Color = 'b';
        ax1.XLim = [min(ai) max(ai)];
        ax1.XTick = [-360:90:360];
        ax1.XLim = [-180 180];
        text(.5,1.09,sprintf('r: %.2f, Max: %.2f, Mean: %.2f, Std: %.2f',sdata.hd_rayleigh,sdata.hd_max,sdata.hd_mean,sdata.hd_stdev),'Units','normalized','FontSize',fsiz,'HorizontalAlignment','center')
        set(gca,'LineWidth',flnw,'layer','top','FontSize',fsiz);   

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ################################################################# %% Head direction (3D) 
    ax2 = axes('Units','pixels','Position',[ax1.Position(1),ax1.Position(2)-260,ax1.Position(3),170]);
        ratemap = sdata.hd_3d_ratemap{1};
        dwellmap = pdata.(part_now).hd_3d_dwellmap;
...
%% ################################################################# %% Speed firing relationship  
    axvs = axes('Units','pixels','Position',[1320,380,200,140]);
        xi = 0:1:50;
...
%% ################################################################# %% Save the figure
    ...
    figname = [figfile sdata.uci{1} '_' part_now '.png'];
    [~,~,~] = mkdir(figfile);
