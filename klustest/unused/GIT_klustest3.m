function GIT_klustest3(tetrodes,clusters,cname)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% ################################################################# %% Prepare important data
    ...
    if ~exist(oname,'file') || angle_override
        rname = 'generic_hpc';
        head_angles = get_3d_orientation(pos,rname); % odat = [X, Y, Z, yaw, pitch, roll]
...
%     poz = mean([rpoz gpoz bpoz],2,'omitnan'); % rat Z center of mass
    pox = head_angles.headx(:); % X center of the rat's head 
    poy = head_angles.heady(:); % Y center of the rat's head 
    poz = head_angles.headz(:); % Z center of the rat's head 
...
%% ################################################################# %% For each part  
            ...
            for pp = 1:nparts % for every partition      
                ...
                if ~isfield(pdata,part_now3)
                    pdata.(part_now3) = pdata.(part_now);
                end
                ...
                % accumulate
                pdata.(part_now3).pox = single(ppox);
                pdata.(part_now3).poy = single(ppoy);
                pdata.(part_now3).poz = single(ppoz);                    
                pdata.(part_now3).pot = single(ppot); 
                pdata.(part_now3).poh = single(ppoh); 
                pdata.(part_now3).pov = single(ppov);
                pdata.(part_now3).duration = part_duration;
...
%% ################################################################# %% Ratemap, dwellmap, spatial analyses, grid analyses, overdispersion
                ...
                sdatap = addToTable(sdatap,'grid_autocorrelation',cell(1,1));
                sdatap = addToTable(sdatap,'sliced_spatial_info',cell(1,1),'sliced_grid_score',cell(1,1),'projected_spatial_info',cell(1,1),'projected_grid_score',cell(1,1),'flattened_spatial_info',cell(1,1),'flattened_grid_score',cell(1,1));
                sdatap = addToTable(sdatap,'projected_maps',cell(1,1),'projected_amaps',cell(1,1),'flattened_maps',cell(1,1),'flattened_amaps',cell(1,1));                 
                fdatap = table;
                ...
                if (config.fild_asis || config.grid_asis || config.ovrd_asis) && numel(pspx)>minspikes % if we want to analyse place fields, grid cells or overdispersion
                    config3.dwellmap = [];
                    dname = [pwd '\klustest\' cname '\' part_now3 '.dwellmap'];
                    if isfield(pdata.(part_now3),'dwellmap') % if a previously computed dwellmap exists, use this to save time
                        config3.dwellmap = pdata.(part_now3).dwellmap;
                    elseif exist(dname,'file')
                        load(dname,'-mat','dwellmap');
                        config3.dwellmap = dwellmap;
                    end
                    ...
                    config3.data_in = [pdata.(part_now).lx(:) pdata.(part_now).ly(:) pdata.(part_now).lz(:)];
                    [ratemap,dwellmap,~,config3,mapdata] = ndmapDATA([ppox ppoy ppoz],[pspx pspy pspz],config3);
                    pdata.(part_now3).lxrm = mapdata.data_out(:,1);
                    pdata.(part_now3).lyrm = mapdata.data_out(:,2);
                    pdata.(part_now3).lzrm = mapdata.data_out(:,3); 
...
%% ########## %% ratemap analysis
                    ...
                    % accumulate data         
                    pdata.(part_now3).dwellmap = single(dwellmap);
                    if ~exist(dname,'file')
                        save(dname,'dwellmap','-mat');
                    end                    
                    sdatap = addToTable(sdatap,'ratemap',{ratemap},'spatial_info_bsec',spatm.spatial_information,'spatial_info_bspike',spatm.spatial_information_perspike,'mutual_info',spatm.mutual_info,'sparsity',spatm.sparsity,'spatial_coherence',spatm.spatial_coherence);
...
%% ########## %% place field analysis
                    ...
                    [pdata,sdatap,fdatap,nfields] = getPFIELDS3(pdata,sdatap,uci,part_now3);                    
...
%% ########## %% grid characteristic analysis
                    if config.grid_asis          
                        ...
                        % accumulate data
                        sdatap = addToTable(sdatap,'grid_autocorrelation',{automap});
                    end      
                    
%% ########## %% ratemap slice and projection analysis
                    [s_si,s_gs] = sliceRATEMAP(pdata,sdatap);
                    [p_si,p_gs,f_si,f_gs,mmaps,mamaps,fmaps,famaps,pdata] = projectRATEMAP(pdata,sdatap);

                    % accumulate data
                    sdatap = addToTable(sdatap,'sliced_spatial_info',{s_si},'sliced_grid_score',{s_gs},'projected_spatial_info',{p_si},'projected_grid_score',{p_gs},'flattened_spatial_info',{f_si},'flattened_grid_score',{f_gs});
                    sdatap = addToTable(sdatap,'projected_maps',{mmaps},'projected_amaps',{mamaps},'flattened_maps',{fmaps},'flattened_amaps',{famaps}); 
                end
...
%% ################################################################# %% HD analyses    
                ...
                % preallocate - in order to concatenate tables after each loop they have to have the same columns, unfortunately this means all variables have to be preallocated in case they are not filled during the loop, the syntax here should be straightforward though if you want to add more
                sdatap = addToTable(sdatap,'hd_spikemap',cell(1,1),'hd_ratemap',cell(1,1),'hd_dwellmap',cell(1,1),'hd_rayleigh',NaN,'hd_max',NaN,'hd_mean',NaN,'hd_stdev',NaN,'hd_3d_ratemap',cell(1,1));

                if config.head_asis && numel(pspx)>minspikes
                    % Load a head direction dwell map if one exists, or create it if not
                    if isfield(pdata.(part_now3),'hd_dwellmap')
                        hd_dwellmap = pdata.(part_now3).hd_dwellmap;
                        [~,~,hd_spikemap,hd_ratemap,r,mx,mn,sd] = mapHD(hd_dwellmap,ppoh,psph,config);
                    else
                        [~,hd_dwellmap,hd_spikemap,hd_ratemap,r,mx,mn,sd] = mapHD([],ppoh,psph,config);
                        pdata.(part_now3).hd_dwellmap = single(hd_dwellmap);
                    end                

%% ########## %% 3D heading direction
                    [rmapHD,dmapHD,~] = analyseHEADING(pdata,sdatap);                 
                    pdata.(part_now3).hd_3d_dwellmap = single(dmapHD);   

                    % accumulate data
                    sdatap = addToTable(sdatap,'hd_spikemap',{single(hd_spikemap(:))},'hd_ratemap',{single(hd_ratemap(:))},'hd_dwellmap',{single(hd_dwellmap(:))},'hd_rayleigh',r,'hd_max',mx(1),'hd_mean',mn,'hd_stdev',sd,'hd_3d_ratemap',{single(rmapHD)});
                end
...
%% ################################################################# %% Speed modulation analysis   
                ...
                if config.sped_asis && numel(pspx)>minspikes
                    ...
                    % accumulate data
                    pdata.(part_now3).speed_dwell_time = single(interp1(svals,stime,0:1:50,'linear'));     
...
%% ################################################################# %% Part figure and finish loop
                ...
                if numel(pspx)>minspikes
                    if run_figPART
                        figPART3(mtint,pdata,sdatap,fdatap); % overall figure with ratemaps etc
                    end
                end
% keyboard
...
%                     figCLUS(mtint,pdata,sdatac); % overall figure with ratemaps etc
                end 
            end
