...
%% >>>>>>>>>> All fields
    ...
    ax1 = axes('Units','pixels','Position',[xnow ynow+50 220 90]);
        ah = add_panel_title('a',sprintf('Example fields'),'yoffset',10,'xoffset',45,'width',400);  
        % ellipses
        f = datn{1};
        alph = 0.3;
        nfields = 150;
        ...
        for ff = 1:nfields
            ...
            x = e.XData;
            y = e.YData;
            patch(x,y,-(f(rindx(ff),3)),'EdgeColor','none','FaceAlpha',alph,'Clipping','off'); hold on;
            delete(e);
        end
        ...
        % text
        text(0,1.01,sprintf('%s, %d random fields',maze_names{1},nfields),'Units','normalized','HorizontalAlignment','left','FontSize',8,'Color','k','VerticalAlignment','bottom','rotation',0)
    ...
    ax2 = axes('Units','pixels','Position',[ax1.Position(1)+xbuff ax1.Position(2) ax1.Position(3) ax1.Position(4)]);
        ...
        % text
        text(0,1.01,sprintf('%s, %d random fields',maze_names{2},nfields),'Units','normalized','HorizontalAlignment','left','FontSize',8,'Color','k','VerticalAlignment','bottom','rotation',0)
    ...
    % example ellipses
    ax2 = axes('Units','pixels','Position',[xnow+300 ynow+ax1.Position(4)+70 130 130],'Color','none');
        ax2.XLim = [0 1.2];
        daspect([1 1 1])
        axis manual off
        ax2.Clipping = 'off';
        ...
        nf = 6;
        xvals = linspace(0,1.2,nf);
        ovals = linspace(0,90,nf);
        ye = 0;   
        ...
        for xx = 1:length(xvals)
            ...
            e = ellipse(emax,emin,-deg2rad(ovals(xx)),xvals(xx),ye(1));
            x = e.XData;
            y = e.YData;
            patch(x,y,ovals(xx),'EdgeColor',[.5 .5 .5],'FaceAlpha',alph,'Clipping','off'); hold on;
            delete(e);
            text(xvals(xx),0.1,sprintf('%.f%c',ovals(xx),176),'FontSize',8,'HorizontalAlignment','center','VerticalAlignment','bottom')
        end
        text(-0.15,ye,sprintf('Fields and orientation:'),'FontSize',8,'HorizontalAlignment','right','VerticalAlignment','bottom')            
...
%% >>>>>>>>>> elongation (distribution)
    ...
    % create axis
    ax = axes('Units','pixels','Position',[xnow ynow+15 270 125]);
        ah = add_panel_title('b',sprintf(''),'yoffset',-10,'xoffset',-10,'width',400);  
        ...
        [~,leg] = legendflex([a1 a2],{maze_names{1},maze_names{2}},'anchor',{'nw','nw'},'ncol',1,'box','off','buffer',[0,-10],'xscale',.5,'fontsize',9); 
        leg(3).Children.FaceAlpha = a1.FaceAlpha;
        leg(4).Children.FaceAlpha = a2.FaceAlpha;

    % example ellipses
    ax2 = axes('Units','pixels','Position',[ax.Position(1) ynow ax.Position(3) ax.Position(3)],'Color','none');
        ax2.XLim = ax.XLim;
        daspect([1 1 1])
        axis manual off
        ax2.Clipping = 'off';
        ...
        for xx = 1:length(xvals)
            ...
            e = ellipse(emax,emin,deg2rad(0),xvals(xx),ye(1));
            x = e.XData;
            y = e.YData;
            patch(x,y,'w','EdgeColor','k','FaceAlpha',1,'Clipping','off'); hold on;
            delete(e);
            text(xvals(xx),ye,sprintf('%.1f',xvals(xx)),'FontSize',8,'HorizontalAlignment','center','VerticalAlignment','middle')
        end

%% >>>>>>>>>> elongation (means)
    xnow = xnow+ax.Position(3)+50;

    ax = axes('Units','pixels','Position',[xnow ynow+15 100 125]);
        [ds,gs] = vectorDATAGROUP([],v1(:),v2(:),v3(:)); % linearise data

        % main plot
        meanplot(ds,gs,'linecolor',{'k'},'meancolor',{'k'},'meansize',8,'meanlinewidth',1,'meanmarker',{'o'},'meanfill',1,'dotcolor',plot_set(1,[1 2 2]),'dotsize',10,'dotalpha',0.5,'dotsmarker',plot_set(2,[1 2 2]),'dotsigma',0.1,'dots',0);

        % axis settings
        ylabel(sprintf('Elongation'))    
        ax.XTick = 1:3;
        ax.XLim = [0.5 3.5]; 
        ax.FontSize = 8;
        ...
        text(5/6,-0.01,sprintf('%s (flattened)',maze_names{2}),'FontSize',8,'HorizontalAlignment','right','Rotation',25,'Units','normalized','VerticalAlignment','top')

        text(jj,0.1,sprintf('Coefficient\nof variation:'),'HorizontalAlignment','center','VerticalAlignment','bottom','FontSize',ax.FontSize)        
        for jj = 1:3
            c = nancoefv(ds(gs==jj));
            text(jj,0.1,sprintf('%.2f',c),'HorizontalAlignment','center','VerticalAlignment','top','FontSize',ax.FontSize)
        end
...
%% >>>>>>>>>> Example cell (fields aligned to maze)
    ...
    ax1 = axes('Units','pixels','Position',[xnow ynow+80 120 70]);
        ah = add_panel_title('c',sprintf('Example cell'),'yoffset',-20,'xoffset',10,'width',400);  
    ...
    % horizontal colormap
    axc = axes('Units','pixels','Position',[xnow+10 ynow-10 70 8]);
        x = linspace(ax1.CLim(1),ax1.CLim(2),100);
        imagesc(x,'XData',ax1.CLim,'YData',[0 1]);
        colormap(axc,ax1.Colormap);
        axis on
...
% cell 1
    ...
    % plot arena
    ax1 = axes('Units','pixels','Position',[xnow ynow+80 120 70]);
        ah = add_panel_title('d',sprintf('Example cell'),'yoffset',-15,'xoffset',10,'width',400);  
...
%% >>>>>>>>>> orientation analysis results        
    ...
    ax = axes('Units','pixels','Position',[xnow ynow 210 100]);       
        % collect data
        ucis = unique(clumaa.uci(pidx));
        datn = cell(1,3);
        for pp=1:3    
    ...
    % example ellipses
    ax2 = axes('Units','pixels','Position',[ax.Position(1) ynow-75 ax.Position(3) ax.Position(3)],'Color','none');
        ah = add_panel_title('e',sprintf(''),'yoffset',-5,'xoffset',-15,'width',400);  
        ...
        for xx = 1:length(xvals)
            ...
            patch(x,y,'w','EdgeColor','k','FaceAlpha',1,'Clipping','off'); hold on;
            delete(e);
            text(xvals(xx),ye,sprintf('%.f',xvals(xx)),'FontSize',8,'HorizontalAlignment','center','VerticalAlignment','middle')
        end

        arrow([-103 70],[-63 70],'FaceColor',[.5 .5 .5],'Length',5,'EdgeColor',[.5 .5 .5]);
        arrow([-103 70],[-103 110],'FaceColor',[.5 .5 .5],'Length',5,'EdgeColor',[.5 .5 .5]);
        text(-103,110,'Y','HorizontalAlignment','center','VerticalAlignment','bottom','FontSize',8,'Color',[.5 .5 .5])
        text(-63,70,'X','HorizontalAlignment','left','VerticalAlignment','middle','FontSize',8,'Color',[.5 .5 .5])
...
%% >>>>>>>>>> Average fields
    ...
    ax1 = axes('Units','pixels','Position',[xnow ynow+80 siz siz]); % arena 1
        ah = add_panel_title('f',sprintf('Avg. field shape'),'yoffset',0,'xoffset',0,'width',400);  
...
% %% >>>>>>>>>> wall alignment analysis predictions
%     xnow = xnow+310;
%     ynow = ynow;
% 
%     % create axis
...
%% >>>>>>>>>> wall alignment analysis results     
    ...
    % create axis
    ax = axes('Units','pixels','Position',[xnow ynow 180 100]);            
        % arena 1
        f1 = datn{1};
        ...
        k = 0.1;
        nbins = 30;
        xi = linspace(0,90,nbins);         
        % k1 = ksdensity(deg2rad(v1),deg2rad(xi),'BandWidth',k);
        % k2 = ksdensity(deg2rad(v2),deg2rad(xi),'BandWidth',k);
        % k3 = ksdensity(deg2rad(v3),deg2rad(xi),'BandWidth',k);
        k1 = histcounts(v1,xi,'Normalization','probability');
        k2 = histcounts(v2,xi,'Normalization','probability');
        k3 = histcounts(v3,xi,'Normalization','probability');
        bi = movmean(xi,2,'Endpoints','discard');
        
        % plot data
        alph = 0.5;
        a1 = area(bi,k1,'FaceColor',plot_set{1,1},'EdgeColor','none','FaceAlpha',alph); hold on;
        a2 = area(bi,k2,'FaceColor',plot_set{1,2},'EdgeColor','none','FaceAlpha',alph); hold on;
        a3 = plot(bi,k3,'k','LineStyle','--');
    ...
    % example ellipses
    ax2 = axes('Units','pixels','Position',[xnow ynow-45 180 180],'Color','none');
        ah = add_panel_title('b',sprintf(''),'yoffset',-10,'xoffset',-10,'width',400);  
        ...
        ovals = 0:30:90;
        xvals = linspace(-90,90,numel(ovals));
        ye = 90;   
        cols = winter(length(xvals));
        for xx = 1:length(xvals)
            emin = 8;
            ...
            patch(x,y,'w','EdgeColor','k','FaceAlpha',1,'Clipping','off'); hold on;
            delete(e);
            text(xvals(xx),ye,sprintf('%.f',ovals(xx)),'FontSize',8,'HorizontalAlignment','center','VerticalAlignment','middle')
        ...
        line([-100 100],[70 70],'Color','k')
        text(0,70,'boundary','HorizontalAlignment','right','VerticalAlignment','top','FontSize',8)
